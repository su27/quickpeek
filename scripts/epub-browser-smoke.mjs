...
window.result=(async()=>{
 ...
 for(const kind of ['epub3','epub2']){
   ...
   assert(viewer.label==='1/3 章','spine order');assert(host.querySelector('nav button').textContent.includes('慢下'),'TOC titles');
   ...
   const internal=host.querySelector('a[data-book-path="OPS/text/two.xhtml"]');internal.click();await wait(()=>viewer.label==='2/3 章');
   const choices=host.querySelectorAll('nav button');choices[2].click();choices[0].click();await wait(()=>viewer.label==='1/3 章');
